...
// Remove chart.js imports that are causing errors
import LoadingSpinner from '@/components/ui/LoadingSpinner';

// Dynamic imports with error handling for charts
const ChartComponents = () => {
  const [chartsLoaded, setChartsLoaded] = useState(false);
  const [ChartJS, setChartJS] = useState(null);
  const [Charts, setCharts] = useState({ Doughnut: null, Bar: null, Line: null });
  ...
  return { chartsLoaded, ChartJS, ...Charts };
};
...
const DashboardPage = () => {
  ...
  const { chartsLoaded, Line, Bar, Doughnut } = ChartComponents();
  ...
  // Render a chart or placeholder based on whether the chart libraries are available
  const renderChart = (ChartComponent, chartData, options, height = "h-72") => {
    if (!chartsLoaded || !ChartComponent) {
      return (
        <div className={`${height} flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-lg`}>
          <p className="text-gray-500 dark:text-gray-400">Chart visualization unavailable</p>
        </div>
      );
    }
        ...
        {activeTab === 'overview' ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
              ...
              <div className="mt-6">
                <button className="w-full py-2 bg-primary-50 hover:bg-primary-100 dark:bg-primary-900/20 dark:hover:bg-primary-900/40 text-primary-700 dark:text-primary-400 text-sm font-medium rounded-md transition-colors flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
              ...
              <div className="space-y-4">
                <button className="w-full py-3 bg-primary-600 hover:bg-primary-700 text-white rounded-lg flex items-center justify-center transition-colors">
                  <FiPlus className="mr-2" /> Add New Book
                </button>
                <button className="w-full py-3 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-900 dark:text-white rounded-lg flex items-center justify-center transition-colors">
